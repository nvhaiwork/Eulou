...
public class CallScreenActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			field = PowerManager.class.getClass()
		...
		Intent serviceIntent = new Intent(CallScreenActivity.this,
				EulouService.class);
	...
	private class SinchCallListener implements CallListener {
		...
		public void onCallEnded(Call call) {
			...
			UserInfo callerUser = service.findFriendUserBySinchID(call
					.getRemoteUserId());
			...
			if (call.getDirection() == CallDirection.OUTGOING) {
				...
				if (call.getDetails().getEstablishedTime() == 0) {
					...
					date = new Date(
							call.getDetails().getEstablishedTime() * 1000);
					endCause = call.getDetails().getEndCause() == CallEndCause.HUNG_UP ? "5"
							: "1";
				}
				...
				try {
					requestString = Constants.EULOU_SERVICE_URL
							+ "?action="
							+ Constants.API_CALL_ADD
							+ "&fromuser="
							+ currentUser.getUserID()
							+ "&touser="
							+ callerUser.getUserID()
							+ "&fromname="
							+ URLEncoder.encode(currentUser.getUserName(),
									"UTF-8")
							+ "&toname="
							+ URLEncoder.encode(callerUser.getUserName(),
									"UTF-8") + "&startdate="
							+ URLEncoder.encode(startdate, "UTF-8")
							+ "&enddate=" + URLEncoder.encode(enddate, "UTF-8")
							+ "&callstatus=" + endCause + "&groupdate="
							+ URLEncoder.encode(groupdate, "UTF-8");
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
					Toast.makeText(CallScreenActivity.this, "Not Updated",
				...
				client.get(requestString, new AsyncHttpResponseHandler() {
					...
					public void onFailure(int statusCode, Header[] headers,
						...
						Toast.makeText(
								CallScreenActivity.this.getApplicationContext(),
				...
				item.startDate = dateFormat.format(new Date(call.getDetails()
						.getStartedTime() * 1000));
				...
				if (call.getDetails().getEndCause() == CallEndCause.NO_ANSWER
						|| call.getDetails().getEndCause() == CallEndCause.CANCELED) {
					if (remoteUserDeviceToken != null
							&& !remoteUserDeviceToken.equals("")) {
						String message = String.format("%s : Missed call",
								mUserInfo.getUserName());
						requestString = "";
						try {
							requestString = String
									.format("%s?action=%s&remotedevice=%s&message=%s&payload=%s&sound=&badge=1",
											Constants.EULOU_SERVICE_URL,
											Constants.API_SEND_PUSH,
											remoteUserDeviceToken,
				...
				item.startDate = dateFormat.format(new Date(call.getDetails()
						.getStartedTime() * 1000));
				item.endDate = dateFormat.format(new Date(call.getDetails()
						.getEndedTime() * 1000));
				item.endCause = call.getDetails().getEndCause().getValue();
				item.direction = "incoming";
				item.count = "1";
				item.groupdate = dateFormat1.format(new Date(call.getDetails()
						.getStartedTime() * 1000));
		...
		public void onShouldSendPushNotification(Call call,
			...
			if (pushPair.getPushData().length > 0) {
				...
				String message = String.format("%s : Incoming call",
						mUserInfo.getUserName());
				...
				try {
					...
					requestString = String
							.format("%s?action=%s&remotedevice=%s&message=%s&payload=%s&sound=%s&badge=0",
									Constants.EULOU_SERVICE_URL,
									Constants.API_SEND_PUSH,
									deviceToken,
									URLEncoder.encode(message, "UTF-8"),
									URLEncoder.encode(
											pushPair.getPushPayload(), "UTF-8"),
									URLEncoder.encode(sound, "UTF-8"));
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
