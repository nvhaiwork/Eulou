...
public class SigninActivity extends BaseActivity implements Request.GraphUserCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonFacebook.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Session.openActiveSession(SigninActivity.this, true, permissions, new Session.StatusCallback() {

                            @Override
                            public void call(Session session, SessionState state, Exception exception) {
                                if (exception != null) {
                                    Toast.makeText(SigninActivity.this.getApplicationContext(), exception.getMessage(), Toast.LENGTH_LONG).show();
                                    return;
                                } else {
                                    if (session.isOpened()) {
                                        progressDialog.show();
                                        Request.newMeRequest(session, SigninActivity.this).executeAsync();
                                    }
                                }
                            }
                        });
        ...
        buttonWhatIs.setOnClickListener(new OnClickListener() {
            ...
            @Override
            public void onClick(View v) {
                Intent welcomeIntent = new Intent(SigninActivity.this, WelcomeActivity.class);
                SigninActivity.this.startActivity(welcomeIntent);
                SigninActivity.this.overridePendingTransition(R.anim.fadein, R.anim.fadeout);
                finish();
            }
        ...
        Intent serviceIntent = new Intent(SigninActivity.this, EulouService.class);
    ...
    public void onCompleted(GraphUser user, Response response) {
        if (user == null) {
            ...
            Toast.makeText(getApplicationContext(), "Failed to retrieve facebook user information.", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        String requestString = Constants.EULOU_SERVICE_URL + "?action=" + Constants.API_SIGNIN_FACEBOOK + "&facebookID=" + socialID;
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                try {
                    ...
                    if (!result.equalsIgnoreCase("succeeded")) {
                        if (birthday == null || birthday.equalsIgnoreCase("null")) {
                            birthday = "";
                        }
                        String requestString = Constants.EULOU_SERVICE_URL + "?action=" + Constants.API_REGISTER_FACEBOOK + "&facebookID=" + socialID + "&username=" + URLEncoder.encode(userName, "UTF-8") + "&usermail=" + URLEncoder.encode(userMail, "UTF-8") + "&birthday=" + birthday + "&gender=" + (gender.equalsIgnoreCase("male") ? "1" : "0");
                        AsyncHttpClient client = new AsyncHttpClient();
                        client.get(requestString, new AsyncHttpResponseHandler() {
                                    ...
                                    public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                                        ...
                                        try {
                                            ...
                                            if (!responseObject.getString("result").equalsIgnoreCase("succeeded")) {
    ...
    public void onLoginSuccessWithUser(UserInfo userInfo) {
        SharedPreferences.Editor editor = getSharedPreferences("Eulou", MODE_PRIVATE).edit();
        Gson gson = new Gson();
        ...
        if (userInfo.getPhone() != null && !userInfo.getPhone().equalsIgnoreCase("")) {
            ...
            mainIntent = new Intent(SigninActivity.this, EnterNumberActivity.class);
        }
