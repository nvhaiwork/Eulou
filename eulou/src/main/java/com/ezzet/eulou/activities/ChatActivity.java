...
public class ChatActivity extends BaseActivity implements TextWatcher, OnRefreshListener {
    ...
    private BroadcastReceiver mMesasgeReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (mCurrentClass.equals(ChatActivity.class.getSimpleName())) {
                ...
                int messageEvent = intent.getIntExtra(Constants.INTENT_MESSAGE_EVENT, 0);
                switch (messageEvent) {

                    case Constants.INTENT_RECEIVED_CHAT_MESSAGE:

                        ChatBubbleMessageModel newMessage = (ChatBubbleMessageModel) intent.getExtras().get(Constants.INTENT_NEW_MESSAGE);
                        if (newMessage != null) {
                    ...
                    case Constants.INTENT_SENT_MESSAGE:
                        ...
                        mLastMsgId = intent.getStringExtra(Constants.INTENT_MESSAGE_DELIVERED_ID);
                        break;
                    case Constants.INTENT_DELIVERED_MESSAGE:

                        if (mChatMessages != null && mChatMessages.size() > 0) {

                            String lastMessageId = intent.getStringExtra(Constants.INTENT_MESSAGE_DELIVERED_ID);
                            ChatBubbleMessageModel lastMessage = mChatMessages.get(mChatMessages.size() - 1);
                            if (lastMessage.isFromMy() && lastMessageId.equals(mLastMsgId)) {
    ...
    protected void onCreate(Bundle arg0) {
        ...
        mRecipient = (UserInfo) getIntent().getSerializableExtra(Constants.INTENT_MESAGE_USER_ID);
        if (mRecipient == null) {
        ...
        mRefreshLayout.setColorSchemeResources(R.color.app_color, R.color.white, R.color.app_color, R.color.white);
                    ...
                    public void onGlobalLayout() {
                        ...
                        if (heightDiff > 100) {
                            ...
                            if (!isKeyboardShowing) {
                                ...
                                if (!mMessageEdt.getText().toString().trim().equals("") && !isEditing) {
        ...
        IntentFilter mesageIntentFilter = new IntentFilter(Constants.INTENT_MESSAGE);
        LocalBroadcastManager.getInstance(this).registerReceiver(mMesasgeReceiver, mesageIntentFilter);
    ...
    protected void onDestroy() {
        ...
        LocalBroadcastManager.getInstance(this).unregisterReceiver(mMesasgeReceiver);
    ...
    private String formatDate(String dateStr) {
        ...
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        SimpleDateFormat timeFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm", Locale.ENGLISH);
        dateFormat.setTimeZone(TimeZone.getTimeZone("UTC"));
        Date startDate = new Date();
        try {
    ...
    private class UploadChatHistory extends AsyncTask<Map<String, Object>, Void, Void> {
    ...
    private class MarkupNewMessage extends AsyncTask<ChatBubbleMessageModel, Void, Void> {
    ...
    private class MarkupAllMessage extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
                ...
                List<MessageModel> messageShow = new HelperFunction(ChatActivity.this).getShownMessageHistoryByUser(userId);
                for (MessageModel message : messageShow) {

                    if (message.getToUser().equals(userId) && message.getFromUser().equals(recipientId)) {

                        readFlg.put(message.getMessageId(), true);
                    }
                }

                List<MessageModel> messages = new HelperFunction(ChatActivity.this).getMessageHistoryByUserId(userId);
    ...
    private class LoadMessageHistory extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            List<ChatBubbleMessageModel> tempMessages = new HelperFunction(ChatActivity.this).getChatHistory(requestData);
