...
public class CallDetailActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String URL = String.format(
				"https://graph.facebook.com/%s/picture?width=640&height=640",
	...
	private void updateRecentCalls() {
		...
		try {
			requestString = Constants.EULOU_SERVICE_URL + "?action="
					+ Constants.API_DETAIL_HISTORY + "&fromuser=" + fromuser
					+ "&touser=" + touser + "&groupdate="
					+ URLEncoder.encode(item.groupdate, "UTF-8")
					+ "&callstatus=" + item.endCause + "&direction="
					+ item.direction;
		...
		client.get(requestString, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers,
				...
				try {
					...
					for (int i = 0; i < responseArray.length(); i++) {
						JSONObject historyObject = responseArray
								.getJSONObject(i);
					...
					e.printStackTrace();
					Toast.makeText(
							CallDetailActivity.this.getApplicationContext(),
			...
			public void onFailure(int statusCode, Header[] headers,
				...
				Toast.makeText(CallDetailActivity.this.getApplicationContext(),
	...
	private class RecentCallsListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView != null) {
				...
				layoutItem = (LinearLayout) lf.inflate(R.layout.call_list_item,
						null);
			}

			TextView textViewTime = (TextView) layoutItem
					.findViewById(R.id.textViewTime);
			TextView textViewDirection = (TextView) layoutItem
					.findViewById(R.id.textViewDirection);
			TextView textViewDuration = (TextView) layoutItem
			...
			if (position == 0) {
				...
				} else if (dateFormat1.format(yesterdayDate).equals(
						startDateString)) {
