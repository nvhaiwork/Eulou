...
public class SinchClientService {
    ...
    public void start(Context context, String userName) {
        ...
        mSinchClient = Sinch.getSinchClientBuilder().context(context).userId(userName).applicationKey("86df4c36-a1ac-416c-bc1a-861196f4f5c2").applicationSecret("7DOIMZdX/0CK0IFIj42EPg==").environmentHost("sandbox.sinch.com").build();
        ...
        mMessageClient.addMessageClientListener(new SinchMessageClientListener());
    }
    ...
    private class SinchMessageClientListener implements MessageClientListener {
        ...
        public void onIncomingMessage(MessageClient arg0, Message message) {
            ...
            LogUtil.e("SinchMessageClientListener", "onIncomingMessage : " + message.getTextBody());
            int i = 0;
            String senderName = "";
            ...
            boolean appStatus = CustomSharedPreferences.getPreferences(Constants.PREF_APP_STATUS, false);
            if (appStatus) {

                if (BaseActivity.mCurrentClass != null && BaseActivity.mCurrentClass.equals(ChatActivity.class.getSimpleName())) {
                    ...
                    if (ChatActivity.mRecipient != null) {
                        ...
                        if (ChatActivity.mRecipient.getMainSocial() == Constants.FACEBOOK) {
                            ...
                            if (!ChatActivity.mRecipient.getFacebookID().contains("@fb")) {

                                remoteId = "fb" + ChatActivity.mRecipient.getFacebookID();
                            } else {

                                remoteId = ChatActivity.mRecipient.getFacebookID();
                            }
                        } else if (ChatActivity.mRecipient.getMainSocial() == Constants.TWITTER) {

                            remoteId = "tw" + ChatActivity.mRecipient.getTwitterID();
                        } else if (ChatActivity.mRecipient.getMainSocial() == Constants.INSTAGRAM) {

                            remoteId = "in" + ChatActivity.mRecipient.getInstagramID();
                        } else {

                            remoteId = "un" + ChatActivity.mRecipient.getUserID();
                        }
                    }
                    ...
                    if (remoteId.equals(recId)) {
                        ...
                        if (messageText.equals(iRecId)) {
                            ...
                            // Typing
                            messageIntent.putExtra(Constants.INTENT_MESSAGE_EVENT, Constants.INTENT_TYPING_MESSAGE);
                            sendMessageBroadcast(messageIntent);
                            return;
                        } else if (messageText.equals(iRecId + "-end")) {

                            // End typing
                            messageIntent.putExtra(Constants.INTENT_MESSAGE_EVENT, Constants.INTENT_END_TYPING_MESSAGE);
                            sendMessageBroadcast(messageIntent);
                            return;
                        } else if (messageText.equals(iRecId + "-offline")) {

                            // Update status offline
                            messageIntent.putExtra(Constants.INTENT_MESSAGE_EVENT, Constants.INTENT_USER_GO_OFFLINE);
                            sendMessageBroadcast(messageIntent);
                            return;
                        } else if (messageText.equals(iRecId + "-online")) {

                            // Update status online
                            messageIntent.putExtra(Constants.INTENT_MESSAGE_EVENT, Constants.INTENT_USER_GO_ONLINE);
                            sendMessageBroadcast(messageIntent);
                            return;
                        }
                        ...
                        messageIntent.putExtra(Constants.INTENT_NEW_MESSAGE, newMessage);
                        messageIntent.putExtra(Constants.INTENT_MESSAGE_EVENT, Constants.INTENT_RECEIVED_CHAT_MESSAGE);
                    }
                } else {

                    if (messageText.equals(iRecId) || messageText.contains("-end") || messageText.contains("-offline") || messageText.contains("-online")) {

                        return;
                    }

                    messageIntent.putExtra(Constants.INTENT_PUSH_NEW_MESSAGE_SENDER_ID, recId);
                    messageIntent.putExtra(Constants.INTENT_PUSH_NEW_MESSAGE, senderName + ": " + messageText);
                    messageIntent.putExtra(Constants.INTENT_MESSAGE_EVENT, Constants.INTENT_RECEIVED_MESSAGE);
                }
                ...
                pushIntent.putExtra(Constants.INTENT_MESAGE_USER_ID, pushUserInfo);
                TaskStackBuilder stackBuilder = TaskStackBuilder.create(mContext);
                stackBuilder.addParentStack(MainActivity.class);
                stackBuilder.addNextIntent(pushIntent);
                PendingIntent pendingIntent = stackBuilder.getPendingIntent(0, PendingIntent.FLAG_UPDATE_CURRENT);

                NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(mContext).setContentTitle(pushUserInfo.getUserName()).setSmallIcon(R.drawable.ic_messages).setDefaults(Notification.DEFAULT_ALL).setContentText(messageText).setAutoCancel(true);
        ...
        @Override
        public void onMessageDelivered(MessageClient arg0, MessageDeliveryInfo arg1) {
            // TODO Auto-generated method stub
            messageIntent.putExtra(Constants.INTENT_MESSAGE_EVENT, Constants.INTENT_DELIVERED_MESSAGE);
            messageIntent.putExtra(Constants.INTENT_MESSAGE_DELIVERED_ID, arg1.getMessageId());
            sendMessageBroadcast(messageIntent);
        }

        @Override
        public void onMessageFailed(MessageClient arg0, Message arg1, MessageFailureInfo arg2) {
            // TODO Auto-generated method stub
            messageIntent.putExtra(Constants.INTENT_MESSAGE_EVENT, Constants.INTENT_FAIL_MESSAGE);
            sendMessageBroadcast(messageIntent);
        }
        ...
        public void onMessageSent(MessageClient arg0, Message arg1, String arg2) {
            ...
            messageIntent.putExtra(Constants.INTENT_MESSAGE_EVENT, Constants.INTENT_SENT_MESSAGE);
            messageIntent.putExtra(Constants.INTENT_MESSAGE_DELIVERED_ID, arg1.getMessageId());
            sendMessageBroadcast(messageIntent);
        }

        @Override
        public void onShouldSendPushData(MessageClient arg0, Message arg1, List<PushPair> arg2) {
            // TODO Auto-generated method stub
